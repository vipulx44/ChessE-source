...
int DrawMaterial(const BOARD *pos) {
    ...
    if (pos->NumofPieces[wP] || pos->NumofPieces[bP]) return FALSE;
    if (pos->NumofPieces[wQ] || pos->NumofPieces[bQ] || pos->NumofPieces[wR] || pos->NumofPieces[bR]) return FALSE;
    if (pos->NumofPieces[wB] > 1 || pos->NumofPieces[bB] > 1) {return FALSE;}
    if (pos->NumofPieces[wN] > 1 || pos->NumofPieces[bN] > 1) {return FALSE;}
    if (pos->NumofPieces[wN] && pos->NumofPieces[wB]) {return FALSE;}
    if (pos->NumofPieces[bN] && pos->NumofPieces[bB]) {return FALSE;}
...
int checkresult(BOARD *pos) {
    ...
    if (pos->fiftyMove > 100) {
     printf("1/2-1/2 {fifty move rule (claimed by Impala)}\n"); return TRUE;
    }

    if (ThreeFoldRep(pos) >= 2) {
     printf("1/2-1/2 {3-fold repetition (claimed by Impala)}\n"); return TRUE;
    }

	if (DrawMaterial(pos) == TRUE) {
     printf("1/2-1/2 {insufficient material (claimed by Impala)}\n"); return TRUE;
    }
	...
	if(InCheck == TRUE)	{
	    if(pos->side == WHITE) {
	      printf("0-1 {black mates (claimed by Impala)}\n");return TRUE;
        } else {
	      printf("0-1 {white mates (claimed by Impala)}\n");return TRUE;
        }
    } else {
      printf("\n1/2-1/2 {stalemate (claimed by Impala)}\n");return TRUE;
    }
	return FALSE;
}
...
void XBOARD(BOARD *pos, SEARCHINFO *info) {
...
void CONSOLE(BOARD *pos, SEARCHINFO *info) {

	printf("Welcome to Impala In Console Mode!\n");
	...
	while(TRUE) {
		...
		if(!strcmp(command, "help")) {
			...
			printf("time x - set thinking time to x seconds\n");
